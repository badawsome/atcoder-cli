...
async fn new_project(opt: NewOpt) -> Result<()> {
    ...
    for bin in bins {
        ...
        bin_arr.push({
            ...
            tbl["path"] = toml_edit::value(Path::new("src").join("bin").join(bin_file.clone()).as_os_str().to_str().unwrap());
...
async fn dump(opt: DumpOpt) -> Result<()> {
    ...
    let bins = if !opt.problem_ids.is_empty() {
        ...
        contest_info.problem_ids_lowercase()
    };
    ...
    for bin in bins {
        ...
        let problem = contest_info
            .problem(&bin)
            .with_context(|| format!("Problem `{}` is not contained in this contest", &bin))?;
        ...
        for (i, case) in test_cases.into_iter().enumerate() {
            let test_file_body = &config.project.test_body_template.clone().replace("{{BIN}}", bin).replace("{{INPUT}}", &case.input).replace("{{OUTPUT}}", &case.output).replace("{{CASE}}", (i+1).to_string().as_str());
            test_file_content.push_str(test_file_body);
        }
        fs::write(cwd.join("tests").join(bin_file), test_file_content)?;
    }
