...
async fn new_project(opt: NewOpt) -> Result<()> {
    ...
    for bin in bins {
        bin_arr.push(toml_edit::Table({
            let mut tbl = toml_edit::Value::Table::new();
            tbl["name"] = toml_edit::Item::Value(toml_edit::Value(bin.clone()));
            tbl["path"] = toml_edit::Item::Value(toml_edit::Value(Path::new("src").join("bin").join(bin.clone()).as_os_str().to_str().unwrap()));
            tbl
        }));
        fs::write(
