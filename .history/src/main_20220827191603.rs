...
fn session_file() -> Result<PathBuf> {
    let dir = if let Some(dir) = env::var_os("CARGO_ATCODER_TEST_CACHE_DIR") {
        ...
        dirs::cache_dir().with_context(|| "failed to get cache dir")?.join("cargo-atcoder")
    };
...
async fn new_project(opt: NewOpt) -> Result<()> {
    ...
    let bins = if !opt.bins.is_empty() {
        ...
        match atc.contest_info(&opt.contest_id).await {
            ...
            Err(err) if http::is_http_error(&err, reqwest::StatusCode::NOT_FOUND) => atc
                ...
                .with_context(|| err.context("could not find problem names. please specify names with `--bins`"))?,
            Err(err) => Err(err)?,
        }
    };
    ...
    let stat = Command::new("cargo").arg("new").arg(&opt.contest_id).status()?;
    if !stat.success() {
    ...
    let mut bin_arr = toml_edit::ArrayOfTables::new();
    for bin in bins {
        let mut bin_file = bin.clone();
        bin_file.push_str(".rs");
        bin_arr.push({
            let mut tbl = toml_edit::table();
            tbl["name"] = toml_edit::value(bin.clone());
            tbl["path"] = toml_edit::value(Path::new("src").join("bin").join(bin_file).as_os_str().to_str().unwrap());
            tbl.as_table().unwrap().clone()
        });
        fs::write(dir.join("src").join("bin").join(bin_file), &config.project.template)?;

        // let code = templ.replace("{{HASH}}", &data_encoding::HEXUPPER.encode(&sha2::Sha256::digest(&bin))[0..8]);
        // code.replace("{{BINARY}}", &bin_base64)

        let test_file_content = config.project.test_header_template.clone().replace("{{BIN}}", &bin);
        test_file_content.push_str(&config.project.test_body_template.clone().replace("{{INPUT}}", to));
        fs::write(dir.join("tests").join(bin_file), contents)?;
    }
    ...
    manifest["bin"] = toml_edit::Item::ArrayOfTables(bin_arr);
...
async fn login() -> Result<()> {
    let username = dialoguer::Input::<String>::new().with_prompt("Username").interact()?;
    ...
    for bin in bins {
        ...
        for &cn in opt.case_num.iter() {
            if cn == 0 || cn > test_cases.len() {
                bail!("Case num {} is not found in problem {} samples", cn, problem_id);
            }
        }
        ...
        let mut test_file_content = config.project.test_header_template.clone().replace("{{BIN}}", &bin);
        for (i, case) in tcs {
            let test_file_body = &config.project.test_body_template.clone().replace("{{INPUT}}", &case.input).replace("{{OUTPUT}}", &case.output).replcae("{{CASE}}", i+1);
            test_file_content.push_str(test_file_body);
        }
        fs::write(dir.join("tests").join(bin_file), test_file_content)?;
    }
...
async fn test(opt: TestOpt) -> Result<()> {
    ...
    if passed && opt.submit {
        ...
        atc.submit(contest_id, &problem_id, &String::from_utf8_lossy(&source)).await?;
    }
...
fn test_samples(
    ...
    for &(i, ref test_case) in test_cases.iter() {
        ...
        let ferr = if let Some(ferr) = cmp_res.1 {
            format!(" (abs error: {:<10.3e}, rel error: {:<10.3e})", ferr.abs_error, ferr.rel_error)
        } else {
            "".to_string()
        };

        if !cmp_res.0 {
            println!("test sample {} ... {}{}", i + 1, red.apply_to("FAILED"), ferr);
    ...
    for (case_no, exec_success, output) in fails {
        ...
        if !exec_success {
            println!("{}: exit code: {}", red.apply_to("runtime error"), output.status.code().unwrap_or_default(),);
            println!();
    ...
    if fail_num == 0 {
        ...
        println!("test result: {}. {} passed; {} failed", red.apply_to("FAILED"), test_case_num - fail_num, fail_num);
        println!();
        Ok(false)
    }
}
...
fn cmp_output(reference: &str, out: &str) -> (bool, Option<FloatError>) {
    ...
    for i in 0..ws1.len() {
        ...
        if (is_float(w1) || is_float(w2)) && (is_float(w1) || is_integer(w1)) && (is_float(w2) || is_integer(w2)) {
...
async fn submit(opt: SubmitOpt) -> Result<()> {
    ...
    let test_passed = if opt.skip_test {
        ...
        let test_cases = atc.test_cases(&problem.url).await?.into_iter().enumerate().collect::<Vec<_>>();
...
fn gen_binary_source(
    ...
    let binary_file = metadata.target_directory.join(target).join("release").join(&bin.name);
    ...
    let status = Command::new(program)
        ...
        .current_dir({
            ...
            package.manifest_path.parent().expect("`manifest_path` should end with \"Cargo.toml\"")
        })
        .status()?;
    ...
    if let Ok(upx_path) = which::which("upx") {
        if !no_upx {
            ...
            let status = Command::new(upx_path).arg("--best").arg("-qq").arg(&binary_file).status()?;
    ...
    let code = {
        ...
        let bin_base64 = if column > 0 { split_lines(&bin_base64, column) } else { bin_base64 };

        // let code = templ.replace("{{SOURCE_CODE}}", source_code.trim_end()).replace(
        let code = templ.replace("{{HASH}}", &data_encoding::HEXUPPER.encode(&sha2::Sha256::digest(&bin))[0..8]);
        code.replace("{{BINARY}}", &bin_base64)
    };
...
fn warmup_for(metadata: &Metadata, specs: Option<&[impl AsRef<str>]>) -> Result<()> {
    let members = specs
        .map(|specs| specs.iter().map(|s| metadata.query_for_member(Some(s.as_ref()))).collect())
...
async fn watch_submission_status(atc: Arc<AtCoder>, contest_id: &str, recent_only: bool) -> Result<Option<usize>> {
    let config = read_config()?;
            ...
            for result in results {
                ...
                match result.status {
                    StatusCode::Waiting(code) => {
                        ...
                        match code {
                            WaitingCode::WaitingForJudge => pb.0.set_message("Waiting for judge..."),
                            WaitingCode::WaitingForRejudge => pb.0.set_message("Waiting for rejudge..."),
                        }
                    }
                    ...
                    StatusCode::Progress(cur, total, code) => {
                        ...
                        if let Some(code) = code {
                            ...
                            pb.0.set_message(format!(
                                "{}",
                                if code.accepted() { green.apply_to(&msg) } else { red.apply_to(&msg) }
                            ));
                        } else {
                            pb.0.set_message("");
                        }
...
fn gen_binary(opt: GenBinaryOpt) -> Result<()> {
    ...
    let filename = opt.output.clone().unwrap_or_else(|| PathBuf::from(format!("{}-bin.rs", opt.problem_id)));
...
fn print_full_result(res: &FullSubmissionResult, verbose: bool) -> Result<()> {
    ...
    println!("Date:          {}", DateTime::<Local>::from(res.result.date).format("%Y-%m-%d %H:%M:%S"));
    ...
    let stat = if let Some(code) = res.result.status.result_code() {
        let msg = code.long_msg();
        format!("{}", if code.accepted() { green.apply_to(&msg) } else { red.apply_to(&msg) },)
    } else {
        "N/A".to_string()
    };

    println!("Result:        {}", stat);
    println!("Runtime:       {}", res.result.run_time.as_deref().unwrap_or("N/A"));
    println!("Memory:        {}", res.result.memory.as_deref().unwrap_or("N/A"));

    if res.result.status.result_code().map(|c| !c.accepted()).unwrap_or(false) && !res.cases.is_empty() {
        ...
        for (code, count) in mm {
            ...
            let msg = format!("{}", if code.accepted() { green.apply_to(&msg) } else { red.apply_to(&msg) },);
...
async fn main() -> Result<()> {
    ...
    match Opt::from_args() {
        ...
        Bin(opt) => gen_binary(opt),
        Status(opt) => status(opt).await,
        Dump(opt) => dump(opt).await,
